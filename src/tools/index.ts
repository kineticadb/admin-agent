...
function makeAlterSystemPropertiesTool(session: KineticaSession) {
  return tool(
    "kinetica_alter_system_properties",
    "Change Kinetica configuration via /alter/system/properties. This writes the value into /opt/gpudb/core/etc/gpudb.conf in place (mirroring to persist/gpudb/rank-0/gpudb.conf.bak) — it is a PERSISTENT config file edit, not an in-memory runtime change, and it survives restarts. Say that when asking for approval. kinetica_alter_configuration writes the same file via the host manager; do not use both routes in one investigation. Only the 43 allow-listed names are accepted; unsupported names are rejected before any API call. Blocked for safety: ai_api_key, external_files_directory. Read the returned 'verification' field, which means PERSISTED, not applied: 'confirmed' (read back, matches), 'failed' (did not persist), 'not_reported' (/show/system/properties does not expose it — 7 of the 43; for execution_mode, audit_response, egress_single_file_max_size and system_metadata_retention_period the value is still in gpudb.conf, so verify with kinetica_show_configuration), 'unavailable' (/show unreadable). For tps_per_tom, tcs_per_tom, subtask_concurrency_limit and enable_audit the value persists but the running system ignores it until restarted (measured: zero thread change, no audit output) — the result carries a restart_note; report the value as written and say a restart is required. enable_procs and worker_endpoint_threads are rejected by the endpoint outright.",

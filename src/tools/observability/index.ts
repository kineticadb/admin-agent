...
async function withClient(
  ...
  endpoint: "promUrl" | "lokiUrl",
  service: string,
  fn: (c: ObservabilityClient) => Promise<ToolResult<unknown>>,
): Promise<string> {
  if (!client?.[endpoint]) return applyOutputPipeline(notConfigured(service));
...
function makeTierSnapshotTool(client: ObservabilityClient | undefined) {
  return tool(
    ...
    async (args: TierSnapshotInput) =>
      text(await withClient(client, "promUrl", "Prometheus", (c) => tierSnapshot(c, args))),
...
function makePromQueryTool(client: ObservabilityClient | undefined) {
  return tool(
    ...
    async (args: PromQueryInput) =>
      text(await withClient(client, "promUrl", "Prometheus", (c) => promQuery(c, args))),
    { annotations: { readOnly: true } },
  );
}
...
function makeLokiQueryTool(client: ObservabilityClient | undefined) {
  return tool(
    "kinetica_loki_query",
    "Query the database's STRUCTURED EVENT streams in Loki. This is not a log viewer: with enable_promtail at its default of false, Loki holds no rank log lines at all. It holds events the database emits directly, in five classes — sql (one record per statement with jobid, user, resource_group, elapsed seconds and the full statement text), job (request failures with 'who' attribution and an error code), status (rank status transitions — the stale-rank signal), config (configuration changes), and mode (system mode transitions). Filter with class/severity/source, or pass a raw LogQL 'selector'; 'contains' adds a body substring filter. Severity values observed are info and uerr. Retention is short (hours to days) with no backfill, so this cannot answer questions about last week — and for stack traces or multi-line SQL text you need a support bundle instead, since those live only in the rolling logs.",
    LokiQuerySchema.shape,
    async (args: LokiQueryInput) =>
      text(await withClient(client, "lokiUrl", "Loki", (c) => lokiQuery(c, args))),
    { annotations: { readOnly: true } },
  );
}
...
export function makeObservabilityTools(client: ObservabilityClient | undefined) {
  return [makeTierSnapshotTool(client), makePromQueryTool(client), makeLokiQueryTool(client)];
}

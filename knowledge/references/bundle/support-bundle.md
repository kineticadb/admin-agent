...
- Timestamps carry no zone marker and come from two clocks. Rolling logs (`logs-local/`) hold Kinetica's own stamp — host-local wall time. Loki tails (`logs/rankN.log`) hold Loki's UTC stamp, because the exporter replaced the original. Lexical comparison is chronological only within one family, and the bundle does not record the host's UTC offset. `kinetica_bundle_search_logs` and `kinetica_bundle_log_timeline` report `timestamp_zone` and warn when one result mixes both; when it does, isolate one family (`rank=` / `host_manager=`) rather than ordering across them, and cite the zone beside every timestamp you quote.

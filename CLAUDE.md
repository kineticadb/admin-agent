...
- **`parse-log-line.ts`** — tolerant parser for one Kinetica log line. Handles the core dialect (`TS SEV (pid,tid,r0/ctx) host source:line - msg`) and the component dialect (no `source:line`/`-`). Severities `INFO/WARN/UERR/ERROR/FATAL`. Unparseable lines return `{ message: raw }` (never dropped). `severityRank()` orders severities for `minSeverity` filtering. Timestamps stay as fixed-width strings — lexical comparison is chronological, dodging timezone ambiguity.

...
- **Model**: defaults to `"sonnet"` (SDK resolves shorthand to latest version); operator can override via `--model=<sonnet|haiku|opus>` on the CLI. Valid values come from `SUPPORTED_MODELS` in `agent/run-agent.ts` — CLI validates against that tuple before calling `runAgent`, so downstream code trusts the value is in-range.

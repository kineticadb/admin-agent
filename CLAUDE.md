...
An autonomous diagnostic agent for Kinetica databases, built with the Claude Agent SDK (`@anthropic-ai/claude-agent-sdk`). The agent connects to a live Kinetica instance, investigates issues using 22 live MCP tools (16 diagnostic + 4 mutation + save_report + alter_table_columns) — plus 6 file-backed bundle tools when a support bundle is in play, and 3 observability tools whenever a Prometheus/Loki stats stack is reachable (**31 total**) — and produces structured markdown reports.
...
Creates an in-process MCP server via `createSdkMcpServer()` with the composed tool set — `serverTools = liveTools + bundleTools + observabilityTools + save_report` (up to 31 in a live session: 16 diagnostic + 4 mutation + alter_table_columns + 6 bundle + 3 observability + save_report; 10 in bundle-only mode, where no live tools are constructed but bundle and observability tools remain) — then streams a `query()` call. Uses `makeInteractivePrompt()` — an async generator that yields user messages between agent turns, synchronized by a `TurnGate` (`agent/turn-gate.ts` — promise-based gate that prevents the prompt from appearing before the agent finishes its turn).
...
- **Allowed tools**: explicit list of 16 diagnostic + 3 observability + save_report + alter_table_columns tools (no wildcards — mutation tools intentionally excluded so they fall through to `canUseTool` callback for approval; alter_table_columns is allowed because it implements its own checklist + SQL preview approval)
...
**`tools/observability/` (3 tools):** `kinetica_tier_snapshot` (per-rank/per-tier utilization with an eviction verdict — the first move for any pressure question), `kinetica_prom_query` (arbitrary PromQL, summarized), `kinetica_loki_query` (structured event streams). `catalog.ts` is a **separate** `Record<ObservabilityToolName, ToolCatalogEntry>` typecheck guard, mirroring `tools/bundle/catalog.ts`, so these never leak into the live `ToolName` union. Registered **unconditionally** (the SDK fixes the tool set at `query()` creation), returning a failure that names the env-var fix when no endpoint was found. Available in bundle-only sessions too, on purpose: the stats stack runs on a different host and commonly outlives the cluster the bundle came from.
...
- `DEFAULT_PROMPT_BUDGET_TOKENS = 30_000` — warn threshold (a tripwire, not a hard limit; raised 15_000 → 20_000 on 2026-06-03 and 20_000 → 30_000 on 2026-09-01 — the cached system prompt makes corpus token cost near-zero, so the number bounds always-on material rather than spend; each raise is recorded in the constant's doc comment)

Wired into `runAgent()` immediately after `buildSystemPrompt()`: a `DEBUG`-gated size line plus an **unconditional** over-budget warning to stderr cueing keyword-based playbook selection. Measured baseline (2026-09-01), 6 playbooks + 11 references + 1 bundle reference:

| Configuration                                                       | Tokens      |
| ------------------------------------------------------------------- | ----------- |
| `bundleCapability` omitted — **not a live-session path**            | ~19,325     |
| `"available"`, no observability (live session, no stats stack)      | **~22,145** |
| `"available"` + observability (live session, stats stack reachable) | **~23,002** |
| `"attached"` + observability (bundle attached too)                  | ~22,928     |

**~22,145 is the real live-session floor.** `runAgent()` always passes `"attached"` or `"available"` when a session exists, so the Support Bundle Capability section (~2,820 tokens) is never absent in practice — earlier baselines that reported ~19,325 measured a code path production never takes, and were therefore under-reporting by ~2.8k. Against the former 20,000 threshold every live session was already over budget. The Observability Capability section adds ~857 tokens on top, and renders only when an endpoint was actually reached. Historic figures (all measured the `undefined` way): ~19,136 on 2026-08-31, before `rank-architecture.md` traded its fixed rank-0 RAM figures for read paths; ~17,201 on 2026-08-10, before the measured config-mechanism section landed in `gpudb-conf.md`; ~15,517 with 10 references before `service-management` was added. Note the system prompt is **cached** by the Agent SDK (built once at startup, re-read on every turn), so corpus token cost is near-zero in practice — `runAgent()` emits a `DEBUG`-gated cache-token line in the session summary (`cacheReadTokens > 0` confirms reuse).

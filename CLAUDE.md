...
- **`classify-file.ts`** — maps a bundle-relative path to a `BundleFileKind` (`core-log | component-log | loki-tail | config | os-diag | process-info | version-info | collection-errors | manifest | unknown`) plus rank / service / host / component hints. The host manager (`hm` in `core-gpudb-rolling-hm.log` / `gpudb-exe-hm-*.txt`) is a singleton **service** (port 9300), NOT a rank, so it is tagged `service: "host-manager"` — keeping the `rank` vocabulary numeric so per-line rank filters and the inventory `ranks` list never see a service name. Component log names have all trailing `.log` suffixes stripped (real stats sub-logs ship doubled, e.g. `stats-loki-node2.log.log`). Loki per-rank exports under `logs/` (`rank0.log` … `rankN.log`) are tagged with their numeric `rank`, and `logs/hostmanager.log` with `service: "host-manager"`, so **all** cluster ranks are addressable — not only those whose rolling logs were captured in `logs-local/`. Rotated rolling logs (`core-gpudb-rolling-r0.log.1`) classify as `core-log` for their rank (older history stays searchable rather than falling to `unknown`).
